...
void FPSActor::UpdateActor(float deltaTime) {
    ...
    // Update position of FPS model relative to actor position (little bit to the right)
...
void FPSActor::ActorInput(const InputState& state) {
    ...
    // Mouse movement
    // Get relative movement from SDL (defined in constructor)
    Vector2 mousePos = state.Mouse.GetPosition();
    // SDL_Log("Mouse: %f, %f", mousePos.x, mousePos.y);

    const int maxMouseSpeed = 500;  // Assume mouse movement is usually between -500 and +500
    const float maxAngularSpeed = Math::Pi * 8;  // Rotation/sec at maximum speed
...
void FPSActor::Shoot() {
    ...
    // Spawn a ball
    auto *ball = new BallActor(GetGame());
    ball->SetPlayer(this);
