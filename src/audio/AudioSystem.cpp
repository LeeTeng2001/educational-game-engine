...
bool AudioSystem::Initialize() {
    // Initialize debug logging, the default is very verbose
    ...
    // Load the master banks (must exist and should be loaded first)
...
void AudioSystem::Update(float deltaTime) {
    // Find any stopped event instances and clean them up
    ...
    // Update FMOD (like calculating 3D space)
    mSystem->update();
}


void AudioSystem::LoadBank(const std::string &name) {
    std::string filePath = Game::PROJECT_BASE + name;

    // Prevent loading duplicate
    if (mBanks.find(filePath) != mBanks.end()) {
        return;
    }
    ...
    FMOD_RESULT result = mSystem->loadBankFile(
            filePath.c_str(), // File filePath of bank
    ...
    if (result == FMOD_OK) {
        ...
        SDL_Log("Cannot load bank: %s", filePath.c_str());
    }
}
...
void AudioSystem::UnloadBank(const std::string &name) {
    ...
    auto iter = mBanks.find(filePath);
    if (iter == mBanks.end()) {
        return;
    }
...
void AudioSystem::UnloadAllBanks() {
    // Simpler helper function that clears everything
