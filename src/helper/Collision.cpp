...
float LineSegment::MinDistSq(const Vector3 &point) const {
    // Construct vectors, to understand the cases look at pictures, easy proof
    ...
    if (Vector3::Dot(ab, ac) < 0.0f) {
        ...
        Vector3 p = scalar * ab;
        // Compute length squared of ac - p to avoid expensive length
...
static bool TestSidePlane(float start, float end, float negd, const Vector3 &norm,
