...
void HUD::Draw(Shader *shader) {
    // Crosshair depends on current target
...
void HUD::UpdateCrosshair(float deltaTime) {
    ...
    start = start + dir * 10;  // small offset to avoid collision with FPSActor
    ...
    if (mGame->GetPhysWorld()->SegmentCast(l, info)) {
        // Is this a target?
        // SDL_Log("Collision: %f, %f, %f", info.mPoint.x, info.mPoint.y, info.mPoint.z);
